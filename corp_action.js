  ...
  for (const stock of stocks) {
    try {
      ...
      if (parsedDividendData && parsedDividendData.length > 0) {
        console.log(`Storing ${parsedDividendData.length} dividend entries for "${stock}"`);
        ...
        for (const dividendEntry of parsedDividendData) {
          ...
          if (stored) {
            console.log(`Successfully stored dividend entry (${dividendEntry.exDate}) for "${stock}" in WordPress.`);
        ...
        console.log(`No dividend data found for "${stock}", skipping database storage.`);
      }
